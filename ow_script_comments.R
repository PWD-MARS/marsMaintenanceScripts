# Commenting to better understand code
...
# Sets initial values for log/errors
kill = FALSE
success = FALSE
errorCode = 0
...
#### This is connecting using the RPostgres driver
marsDBCon <- tryCatch({
  dbPool(
...
#### Checks if connection returns a list - Different from GreenIT script
...
#### Returns generic connection error vs actual error
if(kill == TRUE){
...
# 
# #### In log table, adds note that it is gather OW data with hash
# #### Not sure reason for hashing log milestones
...
#### Why would the filepaths be different than the number of last dates?
...
accessdb_latestdates <- left_join(ow_latestdates, accessdb, by = "ow_uid") %>%
  #### Filter ones that don't have a file path or datatable. What is a datatable?
...
#### For each path in accessdb_latestdates
  ...
  tryCatch(
    expr = {
      ...
      accessdb_newdata <- sqlQuery(accessdbCon, accessdb_query, as.is = TRUE) %>%
        ...
        # Turn date into datetime with tz
