---
title: "Update Water in Cellar (WIC) Complaints"
author: "Farshad Ebrahimi and Monica Gucciardi"
...
#Are we running locally or on RStudio Connect?
  ...
  #We will use this to determine the prefix for the GIS Shapefiles
locationCheck <- list.files("/media/mounts/gsi-monitoring")

if(length(locationCheck) == 0){ 
  #We are running locally (ie debugging/developing)
  footprint_dsn <- "//pwdoows/oows/Watershed Sciences/GSI Monitoring/09 GIS Data/Building_Footprint_Shapefile"
  parcel_dsn <- "//pwdoows/oows/Watershed Sciences/GSI Monitoring/09 GIS Data/PWD_PARCELS"
} else{
  #We are running on Connect
  footprint_dsn <- "/media/mounts/gsi-monitoring/09 GIS Data/Building_Footprint_Shapefile"
  parcel_dsn <- "/media/mounts/gsi-monitoring/09 GIS Data/PWD_PARCELS"
}
...
```{r 1. Prepping the data for storage in MARS DB (renaming, deduplicating, type cast etc), include=FALSE, eval=!kill}
...
# 2. Processing New/Changed Work Orders and Comments

## 2.1 Identifying New/Changed WICs & Comments

```{r 2.1 Checking for new WIC complaints, new WIC comments and changed ones, include=FALSE, eval=!kill}
...
Number of New Work Orders: `r colorize(nrow(new_wic), "red")`

Number of Changed Work Orders: `r colorize(nrow(changed_wic), "red")`

Number of New Comments: `r colorize(nrow(new_comment), "red")`

Number of Changed Comments: `r colorize(nrow(changed_comment), "red")`

## 2.2 Write New Work Orders to MARS DB {.hidden .unlisted}

```{r 2.2 Writing workorders to MARS DB, eval= nrow(new_wic) > 0 & !kill, include=FALSE}

# # add new work orders
#   success_wo <- dbWriteTable(mars_con, Id(schema = "fieldwork", table = "tbl_wic_workorder"), new_wic, append= TRUE, row.names = FALSE )
...
```{r 2.3 Updating changed workorders to MARS DB, eval= nrow(changed_wic) > 0 & !kill, include=FALSE}
...
```{r 2.4 Writing New Comments to MARS DB, eval= nrow(new_comment) > 0 & !kill, include=FALSE}
# 
#  # add new comments
#    success_comments <- dbWriteTable(mars_con, Id(schema = "fieldwork", table = "tbl_wic_comment"), new_comment, append= TRUE, row.names = FALSE )
...
```{r 2.5 Updating changed comments to MARS DB, eval= nrow(changed_comment) > 0 & !kill, include=FALSE}
...
```{r 3.1 Attaching Parcels to WIC Workorders based on address, eval= nrow(cw_workorders) > 0, include=FALSE, eval=!kill}
...
pwd_parcels_sf <- st_read(dsn = parcel_dsn, layer = "PWD_PARCELS")%>%
...
```{r 3.2 Attaching Parcels to WIC Workorders based on facility id, eval= nrow(cw_workorders) > 0, include=FALSE, eval=!kill}
...
```{r 3.3 Intersecting WIC XY Coordinates with PWD Parcels, eval= nrow(cw_workorders) > 0, include=FALSE, eval=!kill}
...
## 3.4 Find New/Changed WIC Parcels

```{r 3.4 Binding the WIC parcels, pulling wic parcels from MARS DB and compare, eval= nrow(wicparcel_xy) > 0 | nrow(wicparcel_join_facilityid) > 0 | nrow(wicparcel_join_address) > 0, include=FALSE, eval=!kill}
...
Number of New WIC Parcels: `r colorize(nrow(new_wic_parcel), "red")`

Number of Changed WIC Parcels: `r colorize(nrow(changed_wic_parcel), "red")`

## 3.5 Add WIC Parcels to MARS DB {.hidden .unlisted}

```{r 3.5 Writing new wic parcels to MARS DB , eval= nrow(new_wic_parcel) > 0 & !kill, include=FALSE}
...
```{r 3.6 Updating changed wic parcels in MARS DB , eval= nrow(changed_wic_parcel) > 0 & !kill, include=FALSE}
...
# 4. Add Spatial Polygons (WICs, SMPs)

## 4.1 Identify New WIC Polygons (Property & Footprint)

```{r 4.1 Identifying New WIC Polygons, include=FALSE, eval=!kill}
...
footprint_sf <- st_read(dsn = footprint_dsn, layer = "Building_Footprint") 
...
Number of New WIC Property Lines Geometry: `r colorize(nrow(new_wic_propertyline_sf),"red")`

Number of New WIC Footprints Geometry: `r colorize(nrow(new_wic_footprint_sf), "red")`

## 4.2 Add WIC Polygons to MARS DB {.hidden .unlisted}

### 4.2.1 Add Property Lines {.hidden .unlisted}

```{r 4.2.1 Add WIC Property Lines in MARS DB, eval= nrow(new_wic_propertyline_sf) > 0 & !kill, include=FALSE}
...
```{r 4.2.2 Add WIC Footprints in MARS DB, eval=  nrow(new_wic_footprint_sf) > 0 & !kill, include=FALSE}
...
```{r 4.3 Identify New SMP polygons, include=FALSE, eval=!kill}
...
Number of New SMP Geometry: `r colorize(nrow(new_smp_sf),"red")`

## 4.4 Add New SMP Polygons to MARS DB {.hidden .unlisted}

```{r 4.4 New SMP Polygons WKTs in MARS DB, eval=  nrow(new_smp_sf) > 0 & !kill, include=FALSE}
...
# 5. Detect WICs around Systems

## 5.1 Intersect WIC Parcels with System Polygons and Calculate Distance {.hidden .unlisted}

```{r 5.1 Intersect WIC Parcels with System Polygons and Calculate Distance, include=FALSE, eval=!kill}
...
```{r 5.2 Identify the WIC Construction Status, include=FALSE, eval=!kill}
...
# autmatically assign Non-issue to pre-con wics
...
## 5.3 Identify New/Changed WIC near Systems

```{r 5.3 Identify New/Change SMP-WIC, include=FALSE, eval=!kill}
...
Number of New WICs within 100 ft of Systems: `r colorize(nrow(new_wic_system),"red")`

Number of Changed WICs within 100 ft of Systems: `r colorize(nrow(changed_wic_system), "red")`

## 5.4 Add New SMP-WIC to MARS DB {.hidden .unlisted}

```{r 5.4 Add New System-WIC to MARS DB, eval= nrow(new_wic_system) > 0 & !kill, include=FALSE}
...
```{r Update Changed SMP-WIC in MARS DB, eval= nrow(changed_wic_system) > 0 & !kill, include=FALSE}
###Log: Gathering Baro Data
logMessage <- data.frame(date = as.Date(today()), hash = log_code,
                         milestone = 17,
                         exit_code = NA,
...
## 5.6 Add New Unique Systems to MARS DB for Status Record Keeping {.hidden .unlisted}

```{r 5.6 Add New  Unique Systems to MARS DB for Status Record Keeping, eval= nrow(new_wic_system_status) > 0 & !kill, include=FALSE}
###Log: Writing new WIC Parcels to MARS
logMessage <- data.frame(date = as.Date(today()), hash = log_code,
                         milestone = 18,
                         exit_code = NA,
...
## 5.8 Summary Table of New/Changed WICs near Systems

### 5.8.1 Table 1. New WICs around Systems (100 ft buffer)

```{r Section 5.8.1- Summary Table of New WICs around SMPs, echo=FALSE, results='asis', eval= nrow(new_wic_system)>0}
# Prep the table
new_wic_kable <- new_wic_system %>%
  inner_join(cw_workorders, by = "workorder_id") %>%
  select(`System ID` = system_id, `Work Order ID` = workorder_id , Date = date,`WIC Address` = wic_address, `Dist. Footprint (ft)` = footprint_dist_ft, `Dist. Property (ft)` = property_dist_ft, wic_wo_status_lookup_uid) %>%
  arrange(desc(Date)) %>%
  distinct()
...
### 5.8.2 Table 2. Changed WICs around Systems (100 ft buffer)

```{r Section 5.8.2 - Summary Table of Changed WICs around SMPs, echo=FALSE, results='asis', eval= nrow(changed_wic_system)>0}
# Prep the table
...
```{r Section 6: Close Out Our Database, echo = FALSE}
