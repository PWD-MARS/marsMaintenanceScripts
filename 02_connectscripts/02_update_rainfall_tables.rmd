...
```{r Section 4 - Reading radar data, include = FALSE, eval = all(nrow(new_rawfiles) > 0, !kill)}
  ...
  #for(i in 10){ #DEBUG: only process the tenth file (has a dst fallback, these introduce parsing corner cases to test)
    ...
    finalCurrentData <- currentdata %>%
      ...
      tryCatch(
        ...
        expr = {
          ...
          print(paste("Writing Radar Raw File Path", new_rawfiles$datafile[i]))
          single_rawfile <- new_rawfiles[i, c("filepath", "yearmon")] %>%
            mutate(filepath = basename(filepath)) #Must strip the dirname to be platform agnostic (local or connect)
          
          dbWriteTable(marsDBCon, RPostgres::Id(schema = "admin", table = "tbl_radar_rawfile"), single_rawfile, append= TRUE, row.names = FALSE)
    ...
    new_rawfiles$outcome[i] <- radar_success
