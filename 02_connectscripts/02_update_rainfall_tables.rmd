...
```{r Section 3 - Gathering radar data, include = FALSE}
  ...
  new_rawfiles <- data.frame(filepath = paste(radarFolder, 
    ...
    mutate(yearmon = str_replace(filepath, ".*(\\d{4})-(\\d{2}).*", "\\1\\2"), #Extract the YYYYMM yearmonth from the filename
          ...
          outcome = NA) #Outcome of writing the file to DB
...
```{r Section XXX - Processing new rain events, include = FALSE, eval = !kill}
  ...
  emptytable <- dbGetQuery(marsDBCon, "select count(*) from data.viw_radar_event_latestdates") %>% pull(count)
  
  if(emptytable == 0){ #If we have no defined rain events (ie, we are recalculating everything), pull all the rain
    rain_newdata <- dbGetQuery(marsDBCon, "select dtime, radar_uid, rainfall_in from data.tbl_radar_rain order by radar_uid, dtime asc")
  } else { #Otherwise, grab all data from after the end of the latest event for each radar
    ...
    rain_newdata <- dbGetQuery(marsDBCon, "select rg.radar_uid, rg.dtime, rg.rainfall_in from data.tbl_radar_rain rg left join data.viw_radar_event_latestdates rgel on rg.radar_uid = rgel.radar_uid where rg.dtime > rgel.dtime")
  }

  #Process the rain data into events, based on a 6 hour interevent time and a minimum depth of 0.1 inches
  radar_newevents <- rain_newdata %>% 
    group_by(radar_uid) %>%
    arrange(dtime) %>% 
    ...
    tryCatch(
        ...
        expr = {
          ...
          logMessage <- data.frame(date = as.Date(today()),
                     ...
                     type = paste("Events for Radar Grid Cell", radar_outcomes$radar_uid[i]),
                     hash = logCode)
