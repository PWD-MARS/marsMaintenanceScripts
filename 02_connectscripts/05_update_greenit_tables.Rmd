...
```
#### The goal of this script is to succesfully read smp, system and project data from the `GreenIT` database and write a copy of new and updated records to`mars_prod`.
...
```{r Section 1: Query GreenIT, echo = FALSE}
  ...
  long_cols <- odbc::odbcConnectionColumns(greenit, "vw_greenit_systembestdata") %>%
    ...
    dplyr::filter(data_type == -9,
                  column_size == 0,
                  name != "SMPs") %>% #Data type for a field we do not import was changed in April 2026
    dplyr::arrange(column_size) %>%     #Exclude that field from the longcols filter because we won't find it.
    dplyr::pull(name)
