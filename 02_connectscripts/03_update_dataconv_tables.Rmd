---
title: "Worker Script Report: Populate DataConv Tables"
...
mars <- dbPool(
  ...
  #################################
  if(any(typeof(mars) == "list", !odbc::dbIsValid(dataconv)))
  {
    kill = TRUE
    errorCode = 1
  }
...
log_code <- digest(now()) #Unique ID for the log batches
  ...
  dbWriteTable(mars, RPostgres::Id(schema = "log", table = "tbl_script_dataconv"), logMessage, append = TRUE, row.names=FALSE)
...
# Queries to populate the tables
    ...
    qry_conveyance <- 'select OBJECTID as object_id, LIFECYCLESTATUS as lifecycle_status, FACILITYID as facility_id, COMPONENTID as component_id,SUBTYPE as subtype, COMMENTS as comments from gisad.gswiconveyance where COMPONENTID is not NULL'
    ...
    qry_inspectionruns <- 'select OBJECTID as object_id, LIFECYCLESTATUS as lifecycle_status, FACILITYID as facility_id, PIPE_COMPONENTID as component_id, DIAMETER as diameter_in, PIPESHAPEPWD as pipe_shape, MATERIALPWD as material, SUBTYPE as subtype, SUBTYPEDESC as subtype_description from gisad.gswiinspectionruns where PIPE_COMPONENTID is not NULL'
...
# Grab the tables
    ...
    inspectionruns <- dbGetQuery(dataconv, qry_inspectionruns)
...
# hash the tables
    hash_basin <- basin %>%
        ...
        mutate(facility_id = tolower(facility_id)) %>% #Match our DB's lower case IDs
        select(-temp)
    ...
    hash_inspectionruns <- inspectionruns %>%
        unite("temp", remove = FALSE) %>%
        rowwise() %>%
        mutate(md5hash = digest(temp, algo = 'md5')) %>%
        mutate(facility_id = tolower(facility_id)) %>% #Match our DB's lower case IDs
...
#Query the DB to pull the existing versions
    ...
    db_inspectionruns <- dbGetQuery(mars, "select * from external.tbl_gswiinspectionruns")

```

```{r Section 4 - Identify differnces between tables, include=FALSE}
#Anti join to find what is different
    anti_basin <- anti_join(hash_basin, db_basin)
    anti_blueroof <- anti_join(hash_blueroof, db_blueroof)
    anti_bumpout <- anti_join(hash_bumpout, db_bumpout)
    anti_cistern <- anti_join(hash_cistern, db_cistern)
    anti_drainagewell <- anti_join(hash_drainagewell, db_drainagewell)
    anti_greenroof <- anti_join(hash_greenroof, db_greenroof)
    anti_permeablepavement <- anti_join(hash_permeablepavement, db_permeablepavement)
    anti_planter <- anti_join(hash_planter, db_planter)
    anti_raingarden <- anti_join(hash_raingarden, db_raingarden)
    anti_swale <- anti_join(hash_swale, db_swale)
    anti_tree <- anti_join(hash_tree, db_tree)
    anti_treetrench <- anti_join(hash_treetrench, db_treetrench)
    anti_trench <- anti_join(hash_trench, db_trench)
    anti_wetland <- anti_join(hash_wetland, db_wetland)

    anti_cleanout <- anti_join(hash_cleanout, db_cleanout)
    anti_controlstructure <- anti_join(hash_controlstructure, db_controlstructure)
    anti_conveyance <- anti_join(hash_conveyance, db_conveyance)
    anti_fitting <- anti_join(hash_fitting, db_fitting)
    anti_inlet <- anti_join(hash_inlet, db_inlet)
    anti_manhole <- anti_join(hash_manhole, db_manhole)
    anti_observationwell <- anti_join(hash_observationwell, db_observationwell)
    anti_structure <- anti_join(hash_structure, db_structure)
    anti_inspectionruns <- anti_join(hash_inspectionruns, db_inspectionruns)
...
```{r Section We Are As One - external.tbl_gswiinspectionruns, echo = FALSE, eval = all(!kill, newAssets, nrow(new_inspectionruns) > 0)}
knitr::asis_output(paste("### New assets to add to `external.tbl_gswiinspectionruns`: ",   nrow(new_inspectionruns)))
kable(head(new_inspectionruns))
```
...
if(newAssets){
  ...
  tryCatch(
    expr = {
      dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswibasin"), new_basin, append = TRUE)
      dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswiblueroof"), new_blueroof, append = TRUE)
      dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswibumpout"), new_bumpout, append = TRUE)
      dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswicistern"), new_cistern, append = TRUE)
      dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswidrainagewell"), new_drainagewell, append = TRUE)
      dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswigreenroof"), new_greenroof, append = TRUE)
      dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswipermeablepavement"), new_permeablepavement, append = TRUE)
      dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswiplanter"), new_planter, append = TRUE)
      dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswiraingarden"), new_raingarden, append = TRUE)
      dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswiswale"), new_swale, append = TRUE)
      dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswitree"), new_tree, append = TRUE)
      dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswitreetrench"), new_treetrench, append = TRUE)
      dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswitrench"), new_trench, append = TRUE)
      dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswiwetland"), new_wetland, append = TRUE)

      dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswicleanout"), new_cleanout, append = TRUE)
      dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswicontrolstructure"), new_controlstructure, append = TRUE)
      dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswiconveyance"), new_conveyance, append = TRUE)
      dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswifitting"), new_fitting, append = TRUE)
      dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswiinlet"), new_inlet, append = TRUE)
      dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswimanhole"), new_manhole, append = TRUE)
      dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswiobservationwell"), new_observationwell, append = TRUE)
      dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswistructure"), new_structure, append = TRUE)
      dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswiinspectionruns"), new_inspectionruns, append = TRUE)
    ...
    if(!kill){ #If the write succeeded
      ...
      new_components <- nrow(new_cleanout) + nrow(new_controlstructure) + nrow(new_conveyance) + nrow(new_fitting) + nrow(new_inlet) + nrow(new_manhole) + nrow(new_observationwell) + nrow(new_structure) + nrow(new_inspectionruns)
...
#Set UID to be the last column to facilitate the substitution mechanism in dbSendQuery
    ...
    update_inspectionruns <- anti_join(anti_inspectionruns, new_inspectionruns) %>%
        left_join(select(db_inspectionruns, gswiinspectionruns_uid, facility_id)) 
    
    newUpdates = FALSE #Set true if any of the above objects are nonzero length

    if(any(nrow(update_basin) != 0, nrow(update_blueroof) != 0, nrow(update_bumpout) != 0, nrow(update_cistern) != 0, nrow(update_drainagewell) != 0, nrow(update_greenroof) != 0, nrow(update_permeablepavement) != 0, nrow(update_planter) != 0, nrow(update_raingarden) != 0, nrow(update_swale) != 0, nrow(update_tree) != 0, nrow(update_treetrench) != 0, nrow(update_trench) != 0, nrow(update_wetland) != 0, nrow(update_cleanout) != 0, nrow(update_controlstructure) != 0, nrow(update_conveyance) != 0, nrow(update_fitting) != 0, nrow(update_inlet) != 0, nrow(update_manhole) != 0, nrow(update_observationwell) != 0, nrow(update_structure) != 0, nrow(update_inspectionruns) !=0))
    {
      newUpdates = TRUE
    }
...
```{r Section We Are As One - Updates to external.tbl_gswiinspectionruns, echo = FALSE, eval = all(!kill, newUpdates, nrow(update_inspectionruns) > 0)}
knitr::asis_output(paste("### Assets to update in `external.tbl_gswiinspectionruns`: ",   nrow(update_inspectionruns)))
kable(head(update_inspectionruns))
```
  ...
  tryCatch(
    expr = {
       ...
       if(nrow(update_basin) > 0){
          purgeQuery <- paste0("delete from external.tbl_gswibasin where gswibasin_uid in (", paste(update_basin$gswibasin_uid, collapse = ", "), ");")
          dbExecute(mars, purgeQuery)
          dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswibasin"), update_basin, append = TRUE)
      }
      
      if(nrow(update_blueroof) > 0){
          purgeQuery <- paste0("delete from external.tbl_gswiblueroof where gswiblueroof_uid in (", paste(update_blueroof$gswiblueroof_uid, collapse = ", "), ");")
          dbExecute(mars, purgeQuery)
          dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswiblueroof"), update_blueroof, append = TRUE)
      }
      
      if(nrow(update_bumpout) > 0){
          purgeQuery <- paste0("delete from external.tbl_gswibumpout where gswibumpout_uid in (", paste(update_bumpout$gswibumpout_uid, collapse = ", "), ");")
          dbExecute(mars, purgeQuery)
          dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswibumpout"), update_bumpout, append = TRUE)
      }
      
      if(nrow(update_cistern) > 0){
          purgeQuery <- paste0("delete from external.tbl_gswicistern where gswicistern_uid in (", paste(update_cistern$gswicistern_uid, collapse = ", "), ");")
          dbExecute(mars, purgeQuery)
          dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswicistern"), update_cistern, append = TRUE)
      }
      
      if(nrow(update_drainagewell) > 0){
          purgeQuery <- paste0("delete from external.tbl_gswidrainagewell where gswidrainagewell_uid in (", paste(update_drainagewell$gswidrainagewell_uid, collapse = ", "), ");")
          dbExecute(mars, purgeQuery)
          dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswidrainagewell"), update_drainagewell, append = TRUE)
      }
      
      if(nrow(update_greenroof) > 0){
          purgeQuery <- paste0("delete from external.tbl_gswigreenroof where gswigreenroof_uid in (", paste(update_greenroof$gswigreenroof_uid, collapse = ", "), ");")
          dbExecute(mars, purgeQuery)
          dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswigreenroof"), update_greenroof, append = TRUE)
      }
      
      if(nrow(update_permeablepavement) > 0){
          purgeQuery <- paste0("delete from external.tbl_gswipermeablepavement where gswipermeablepavement_uid in (", paste(update_permeablepavement$gswipermeablepavement_uid, collapse = ", "), ");")
          dbExecute(mars, purgeQuery)
          dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswipermeablepavement"), update_permeablepavement, append = TRUE)
      }
      
      if(nrow(update_planter) > 0){
          purgeQuery <- paste0("delete from external.tbl_gswiplanter where gswiplanter_uid in (", paste(update_planter$gswiplanter_uid, collapse = ", "), ");")
          dbExecute(mars, purgeQuery)
          dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswiplanter"), update_planter, append = TRUE)
      }
      
      if(nrow(update_raingarden) > 0){
          purgeQuery <- paste0("delete from external.tbl_gswiraingarden where gswiraingarden_uid in (", paste(update_raingarden$gswiraingarden_uid, collapse = ", "), ");")
          dbExecute(mars, purgeQuery)
          dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswiraingarden"), update_raingarden, append = TRUE)
      }
      
      if(nrow(update_swale) > 0){
          purgeQuery <- paste0("delete from external.tbl_gswiswale where gswiswale_uid in (", paste(update_swale$gswiswale_uid, collapse = ", "), ");")
          dbExecute(mars, purgeQuery)
          dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswiswale"), update_swale, append = TRUE)
      }
      
      if(nrow(update_tree) > 0){
          purgeQuery <- paste0("delete from external.tbl_gswitree where gswitree_uid in (", paste(update_tree$gswitree_uid, collapse = ", "), ");")
          dbExecute(mars, purgeQuery)
          dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswitree"), update_tree, append = TRUE)
      }
      
      if(nrow(update_treetrench) > 0){
          purgeQuery <- paste0("delete from external.tbl_gswitreetrench where gswitreetrench_uid in (", paste(update_treetrench$gswitreetrench_uid, collapse = ", "), ");")
          dbExecute(mars, purgeQuery)
          dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswitreetrench"), update_treetrench, append = TRUE)
      }
      
      if(nrow(update_trench) > 0){
          purgeQuery <- paste0("delete from external.tbl_gswitrench where gswitrench_uid in (", paste(update_trench$gswitrench_uid, collapse = ", "), ");")
          dbExecute(mars, purgeQuery)
          dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswitrench"), update_trench, append = TRUE)
      }
      
      if(nrow(update_wetland) > 0){
          purgeQuery <- paste0("delete from external.tbl_gswiwetland where gswiwetland_uid in (", paste(update_wetland$gswiwetland_uid, collapse = ", "), ");")
          dbExecute(mars, purgeQuery)
          dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswiwetland"), update_wetland, append = TRUE)
      }
      
      if(nrow(update_cleanout) > 0){
          purgeQuery <- paste0("delete from external.tbl_gswicleanout where gswicleanout_uid in (", paste(update_cleanout$gswicleanout_uid, collapse = ", "), ");")
          dbExecute(mars, purgeQuery)
          dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswicleanout"), update_cleanout, append = TRUE)
      }
      
      if(nrow(update_controlstructure) > 0){
          purgeQuery <- paste0("delete from external.tbl_gswicontrolstructure where gswicontrolstructure_uid in (", paste(update_controlstructure$gswicontrolstructure_uid, collapse = ", "), ");")
          dbExecute(mars, purgeQuery)
          dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswicontrolstructure"), update_controlstructure, append = TRUE)
      }
      
      if(nrow(update_conveyance) > 0){
          purgeQuery <- paste0("delete from external.tbl_gswiconveyance where gswiconveyance_uid in (", paste(update_conveyance$gswiconveyance_uid, collapse = ", "), ");")
          dbExecute(mars, purgeQuery)
          dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswiconveyance"), update_conveyance, append = TRUE)
      }
      
      if(nrow(update_fitting) > 0){
          purgeQuery <- paste0("delete from external.tbl_gswifitting where gswifitting_uid in (", paste(update_fitting$gswifitting_uid, collapse = ", "), ");")
          dbExecute(mars, purgeQuery)
          dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswifitting"), update_fitting, append = TRUE)
      }
      
      if(nrow(update_inlet) > 0){
          purgeQuery <- paste0("delete from external.tbl_gswiinlet where gswiinlet_uid in (", paste(update_inlet$gswiinlet_uid, collapse = ", "), ");")
          dbExecute(mars, purgeQuery)
          dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswiinlet"), update_inlet, append = TRUE)
      }
      
      if(nrow(update_manhole) > 0){
          purgeQuery <- paste0("delete from external.tbl_gswimanhole where gswimanhole_uid in (", paste(update_manhole$gswimanhole_uid, collapse = ", "), ");")
          dbExecute(mars, purgeQuery)
          dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswimanhole"), update_manhole, append = TRUE)
      }
      
      if(nrow(update_observationwell) > 0){
          purgeQuery <- paste0("delete from external.tbl_gswiobservationwell where gswiobservationwell_uid in (", paste(update_observationwell$gswiobservationwell_uid, collapse = ", "), ");")
          dbExecute(mars, purgeQuery)
          dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswiobservationwell"), update_observationwell, append = TRUE)
      }
      
      if(nrow(update_structure) > 0){
          purgeQuery <- paste0("delete from external.tbl_gswistructure where gswistructure_uid in (", paste(update_structure$gswistructure_uid, collapse = ", "), ");")
          dbExecute(mars, purgeQuery)
          dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswistructure"), update_structure, append = TRUE)
      }


      if(nrow(update_inspectionruns) > 0){
          purgeQuery <- paste0("delete from external.tbl_gswiinspectionruns where gswiinspectionruns_uid in (", paste(update_inspectionruns$gswiinspectionruns_uid, collapse = ", "), ");")
          dbExecute(mars, purgeQuery)
          dbWriteTable(mars, RPostgres::Id(schema = "external", table = "tbl_gswiinspectionruns"), update_inspectionruns, append = TRUE)
      }
...
#Write records
if(!kill){ #If the write succeeded
  updated_records <- nrow(update_basin) + nrow(update_blueroof) + nrow(update_bumpout) + nrow(update_cistern) + nrow(update_drainagewell) + nrow(update_greenroof) + nrow(update_permeablepavement) + nrow(update_planter) + nrow(update_raingarden) + nrow(update_swale) + nrow(update_tree) + nrow(update_treetrench) + nrow(update_trench) + nrow(update_wetland) + nrow(update_cleanout) + nrow(update_controlstructure) + nrow(update_conveyance) + nrow(update_fitting) + nrow(update_inlet) + nrow(update_manhole) + nrow(update_observationwell) + nrow(update_structure) + nrow(update_inspectionruns)
...
```{r Section 6: Close Out Database, echo = FALSE}
