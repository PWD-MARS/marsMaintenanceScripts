...
#S.1 Level data import
  prod_ows <- dbGetQuery(mars_old, "select distinct ow_uid from data.tbl_ow_leveldata_raw order by ow_uid asc")
  
  for(i in 1:nrow(prod_ows)){
   print(paste("Pulling OW", prod_ows$ow_uid[i]))
    prod_leveldata <- dbGetQuery(mars_old, paste0("select dtime_est, ow_uid, level_ft, date_added from data.tbl_ow_leveldata_raw where ow_uid = ", prod_ows$ow_uid[i]))
    
    prod_leveldata$dtime_est <- force_tz(prod_leveldata$dtime_est, tz = "EST") #It comes in as UTC, we must make it EST
...
#S.2 Groundwater data import
  ...
  prod_gws <- dbGetQuery(mars_old, "select distinct ow_uid from data.tbl_gw_depthdata_raw order by ow_uid asc")
  
  for(i in 1:nrow(prod_gws)){
    print(paste("Pulling GW", prod_gws$ow_uid[i]))
    prod_depthdata <- dbGetQuery(mars_old, paste0("select dtime_est, ow_uid, depth_ft from data.tbl_gw_depthdata_raw where ow_uid = ", prod_gws$ow_uid[i]))
