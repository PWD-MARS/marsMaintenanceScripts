---
title: "Update Water in Cellar (WIC) Complaints App-Version 2.0"
...
```{r 2.2 Writing workorders to MARS DB, eval= params$write == TRUE & nrow(new_wic) > 0 & !kill, include=FALSE}
...
```{r 2.3 Updating changed workorders to MARS DB, eval= params$write == TRUE & nrow(changed_wic) > 0 & !kill, include=FALSE}
...
```{r 2.4 Writing New Comments to MARS DB, eval= params$write == TRUE & nrow(new_comment) > 0 & !kill, include=FALSE}
# 
#  # add new comments
...
```{r 2.5 Updating changed comments to MARS DB, eval= params$write == TRUE & nrow(changed_comment) > 0 & !kill, include=FALSE}
...
```{r 3.5 Writing new wic parcels to MARS DB , eval= params$write == TRUE & nrow(new_wic_parcel) > 0 & !kill, include=FALSE}
...
```{r 3.6 Updating changed wic parcels in MARS DB , eval= params$write == TRUE & nrow(changed_wic_parcel) > 0 & !kill, include=FALSE}
...
```{r 4.2.1 Add WIC Property Lines in MARS DB, eval= params$write == TRUE & nrow(new_wic_propertyline_sf) > 0 & !kill, include=FALSE}
...
```{r 4.2.2 Add WIC Footprints in MARS DB, eval= params$write == TRUE & nrow(new_wic_footprint_sf) > 0 & !kill, include=FALSE}
...
```{r 4.4 New SMP Polygons WKTs in MARS DB, eval= params$write == TRUE & nrow(new_smp_sf) > 0 & !kill, include=FALSE}
...
```{r 5.4 Add New System-WIC to MARS DB, eval= params$write == TRUE & nrow(new_wic_system) > 0 & !kill, include=FALSE}
...
```{r Update Changed SMP-WIC in MARS DB, eval= params$write == TRUE & nrow(changed_wic_system) > 0 & !kill, include=FALSE}
  ###Log: Gathering Baro Data
  logMessage <- data.frame(date = as.Date(today()), hash = log_code,
                           milestone = 17,
                           exit_code = NA,
 ...
 # update changed wic-smp
   for (i in 1:nrow(changed_wic_system)) {
      ...
      changed_wic_system$footprint_dist_ft[i],
      changed_wic_system$property_dist_ft[i],
...
```{r 5.6 Add New  Unique Systems to MARS DB for Status Record Keeping, eval= params$write == TRUE & nrow(new_wic_system_status) > 0 & !kill, include=FALSE}
  ###Log: Writing new WIC Parcels to MARS
  logMessage <- data.frame(date = as.Date(today()), hash = log_code,
                           milestone = 18,
                           exit_code = NA,
...
## 5.7 Add New Unique Workorder ids to MARS DB for Status Record Keeping {.hidden .unlisted}

```{r 5.7 Add New  Unique Workorder ids to MARS DB for Status Record Keeping, eval= params$write == TRUE & nrow(new_wic_wo_status) > 0 & !kill, include=FALSE}
  ###Log: Writing new WIC Parcels to MARS
  logMessage <- data.frame(date = as.Date(today()), hash = log_code,
                           milestone = 19,
                           exit_code = NA,
